...
public class GetPosition : MonoBehaviour // 该类用于实时获取位置、输出json
{
    ...
    public void receiveMessageInPosition(string info)
    {
        ...
        else if (info == "sunny" || info == "rainy" || info == "snowy" || info == "foggy")
        {
    ...
    IEnumerator getPosition()
    {
        ...
        while (true)
        {
            ...
            /*angle = GameObject.FindGameObjectWithTag("VROrigin").GetComponent<Transform>().localEulerAngles.y;*/ // 使用软件内目标XROrigin获取的角度，方法已遗弃
            // 获取旋转角
            Position_x_y.rotate = oular;
            PositionData.positionList.Add(Position_x_y);
            Position_x_y.x = xx-536;// 原点坐标在世界坐标系中是（X: 536，z: 452）
            Position_x_y.z = zz-452;
